...
import { getMemberRoutines } from "@/app/actions/routines"
import ERTableWrapper from "../components/ERTableWrapper"
import { PartialRoutine } from "../components/ERTableWrapper"
...
export default async function Page({ params }: { params: { memId: number } }) {
    ...
    const routines = await getMemberRoutines(params.memId);
    ...
    const routiness: PartialRoutine[] = [];

    //loop through routines building PartialRoutine objects
    for (let index = 0; index < routines.length; index++) {
      const element = routines[index];
      const rt: PartialRoutine = {
        routine_id: element.routine_id as number,
        routine_name: element.routine_name,
        routine_type: element.routine_type,
        routine: element.routine as JSON,
        difficulty_level: element.difficulty_level,
        routine_description: element.routine_description as string,
      }
      routiness.push(rt);
    ...
    return (
        <div className="grid grid-col-3 mt-16">
            ...
            <ProfileForm memberId={params.memId} />
            </div>
